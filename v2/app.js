...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btt) {
        window.addEventListener('scroll', () => {
            ...
            // Bento Grid Float Animation Toggle
            const servicesSection = document.getElementById('services');
            if (servicesSection && document.body.getAttribute('data-view') === 'desktop') {
                const rect = servicesSection.getBoundingClientRect();
                const vh = window.innerHeight;
                // Add animation class if section is partially visible
                const isVisible = rect.top < vh && rect.bottom > 0;

                const cards = document.querySelectorAll('.v2-desktop-split .service-card');
                cards.forEach(card => {
                    if (isVisible) {
                        card.classList.add('scrolling-anim');
                    } else {
                        card.classList.remove('scrolling-anim');
                    }
                });
            }
        });
